...
class SemanticAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        #     y = x
        # if chosen == 1:
        #     y1 = torch.nn.AdaptiveMaxPool1d(64)
        #     y = y1(x)
        #     y2 = torch.nn.AdaptiveMaxPool1d(128)
        #     y = y2(y)
...
class SeMaskSwinTransformer(nn.Module):
    ...
    '''   
     #new
        ...
        #self.para = nn.Parameter(torch.tensor([1/2], device=device))
        self.para = nn.Parameter(torch.tensor([1.0 for j in range(self.num_layers)], device=device))
        #self.para = nn.Parameter(torch.tensor([pow(0.5,j) for j in range(self.num_layers)], device=device))
    '''    
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                '''
                device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
                para = nn.Parameter(torch.tensor([1/2], device=device))
                cls_outs.append(pow(para, i) * cls_out)
                '''
                '''
                cls_outs.append(self.para[i] * cls_out)
                print(self.para)
                '''
                '''
                print(self.para)
                cls_outs.append(pow(self.para, i) * cls_out)
                '''
                #cls_outs.append(cls_out)
